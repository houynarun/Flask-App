...
<div class="detailReview">
	...
	{%set ReviewType = ReviewDict[FollowUpObj.ReviewType] if FollowUpObj else ''%}
...
<script>
	$(document).ready(function(){
		...
		window.parent.$('#CardID,#CustomerID').change(function(){
			// alert($(this).val());
			if($(this).val()!='__None'){
			setTimeout(getCFUID, 1000 );
			}
		})

		// function to fetch review detail
		function getCFUID () {
			var followupid = window.parent.$('#CardID').val();
			var customerid = window.parent.$('#CustomerID').val();
			// alert('follid:'+followupid+', customerid: '+customerid);
			var url = '';

			if (followupid =='__None' && customerid=='__None')
				// alert('followup is none and customer is none')
				return 

			if (followupid != '__None')
			{
				// alert('followupid is not empty');
				url = "/Morakot/getDetail/"+followupid;
			}else
			{
				// alert('followupid is empty');
				url = "/Morakot/getDetail/"+customerid;
			}
			// alert(url);
			// alert(followupid);
			...
			$.ajax({
				...
				success: function(data){
					$('div.detailReview').html(data);
					// console.log(data);
				},
				error:function(data){
					// var CustomerID = window.parent.$('#CustomerID').val();
					// var ms = 'Customer ID '+CustomerID+' does not have create followup card yet';
					// alert('Customer not yet created followup card yet');
					location.reload();
				}
			});
		}

		// Fuction to load detail review on list view
		var Curr = window.parent.$('body').find('#Curr').val();
		// alert(Curr);
		if(Curr!='')
		{
			setTimeout(getCFUID,1000);
