					...
					<tbody>
						{% if LoanApplicationObj %}
							...
							<tr title="{{Title_ToolTip}}">
								<td class="link">{{LoanApplication.Branch}}</td>
								<td>{{LoanApplication.ID}}</td>
								<td onClick="{{LinkDetail}}" class="link">{{LoanApplication.LNCustomerID}}</td>
								<td onClick="{{LinkDetail}}" class="link">{{getCustomerName(LoanApplication.LNCustomerID)}}</td>
								<td onClick="{{LinkDetail}}" class="link">{{getCustomerName(LoanApplication.LNCustomerID,'Kh')}}</td> 
								<td onClick="{{LinkDetail}}" class="link">{{toMoney(float(LoanApplication.Amount),CurrencyObj(LoanApplication.Currency),2)}}</td>
								<td onClick="{{LinkDetail}}" class="link">{{LoanApplication.AppDate}}</td>
								<td onClick="{{LinkDetail}}" class="link">{{LoanApplication.InterestRate}}</td>
								<td onClick="{{LinkDetail}}" class="link">{{LoanApplication.Term}}</td>
								<td onClick="{{LinkDetail}}" class="link">{{getUserName(LoanApplication.PreparedBy)}}</td>
								<td onClick="{{LinkDetail}}" class="link">
									{%for row in Approval%}
										{{getUserName(row.Inputter)}}
										{%if not loop.last%}
										,&nbsp;
