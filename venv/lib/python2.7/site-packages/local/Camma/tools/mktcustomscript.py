...
class CustomPenalty(object):
	...
	def setCustomPretermination(**kwargs):
		...
		try:
			...
			RemainingInt = 0
			if ScheduleObj:
				RemainingInt 	= ScheduleObj.Interest - AccrCurrentInt
			TotalPenalty 	= RemainingInt * Decimal(Perc)
			TotalPenalty 	= mktmoney.toMoney(float(TotalPenalty),mktmoney.getCurrencyObj(LoanObj.Currency),DecimalPlace=0,RndMode=3)
