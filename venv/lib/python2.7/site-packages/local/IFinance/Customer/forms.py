...
class FRM_CUSTOMER_EX(FRM_CUSTOMER):
	...
	ActiveMember 		=	TextField('# Active Member', description="3-Family Details",
								validators=[validators.Length(max=3)])
	...
	EmployerName 		=	TextField(requiredlabel("Employer Name","*"),
							description="4-%s" %getLanguage('Employment Details'),
	...
	MonthlyIncome 		=	TextField(requiredlabel("Total Monthly Salary/Income","*"),
							description="4-%s" %getLanguage('Employment Details'),
							validators=[validators.Required()])
	...
	def validate_MonthlyIncome(form,field):
		try:
			...
			print type(MonthlyIncome),'Monthly'
			if MonthlyIncome == '0':
