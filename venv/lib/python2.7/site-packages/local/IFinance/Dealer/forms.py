...
def getRecord():
	try:
		result = MKT_DEALER_INAU.query.filter_by(ID=g.formID).all()
		if not result:
			result = MKT_DEALER.query.filter_by(ID=g.formID).all()
		Dict={}
		if result:
...
class FRM_DEALER(exform):
	...
	# --Contact and Address--
	Province 			= 		QuerySelectField('Province',query_factory=loadProvince, 
									 get_label='Description',
									 allow_blank=True,
									 blank_text=u'--Choose Province--',
									)
	District 		=	 	QuerySelectField('District',query_factory=loadDistrict, 
									 get_label='Description',
									 allow_blank=True,
									 blank_text=u'--Choose District--',
									)
	Commune 			=		QuerySelectField('Commune',query_factory=loadCommune, 
									 get_label='Description',
									 allow_blank=True,
