...
{% block content %}
    ...
    <div class="col-sm-12">
        <table id="mktlist" class="table table-striped table-bordered">
            <thead>
                <tr style="border: 0;">
                    <th colspan="6" class="text-right" style="border: none;">({{ getLang(Language, '110009') }})</th>
                </tr>
                <tr>
                    <th rowspan="2" style="background: {{ Result['ColumnHeader']['Color'] }}; font-weight: bold;"></th>
                    {% if InDollar == 'Y' %}
                        <th colspan="4" class="text-center" style="background: {{ Result['ColumnHeader']['Color'] }}; font-weight: bold;">
                            {% if Period == 'M'%}
                                ប្រចាំខែ
                            {% else %}
                                {{ getLang(Language, '110011') }}
                            {% endif %}
                        </th>
                    {% else %}
                        <th colspan="3" class="text-center" style="background: {{ Result['ColumnHeader']['Color'] }}; font-weight: bold;">
                            {% if Period == 'M'%}
                                ប្រចាំខែ
                            {% else %}
                                {{ getLang(Language, '110011') }}
                    ...
                    {% endif %}
                    <th rowspan="2" valign="top" class="text-center" style="background: {{ Result['ColumnHeader']['Color'] }}; font-weight: bold;">{{ getLang(Language, '110013') }}</th>
                </tr>
                <tr>
                    {% if InDollar == 'Y' %}
                        <th style="background: {{ Result['ColumnHeader']['Color'] }}; font-weight: bold;">{{ getLang(Language, '110005') }}</th>
                    {% endif %}
                    <th style="background: {{ Result['ColumnHeader']['Color'] }}; font-weight: bold;">{{ getLang(Language, '110012') }}</th>
                    ...
                    {% if item.Header == 1 %}
                        ...
                        {% elif item.LineType == 'GH' %}
                            <tr>
                                ...
                                <td style="background: {{ item.RowBGColor }}; font-weight: {{ item.LineFormat }};">{{ valueDisplay(item.YTDBalance) }}</td>
                            </tr>
                        ...
                        {% else %}
                            <tr>
                                ...
                                <td>{{ valueDisplay(item.YTDBalance) }}</td>
                            </tr>
                        {% endif %}
                    {% endif %}
                {% endfor %}
...
{% block js %}
    ...
    <script type="text/javascript">
        ...
        $(function(){
            ...
            var PeriodID = "{{ Period }}";
            $("select#PeriodID option").filter(function() {
                //may want to use $.trim in here
                return $(this).val() == PeriodID;
            }).prop('selected', true);
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form class="form-horizontal" action="/Morakot/Report/N002/" method="GET">
                    ...
                    <div class="modal-body">
                        ...
                        <!-- <div class="form-group">
                            <label for="Period" class="col-sm-4 control-label">Select Period:</label>
                            <div class="col-sm-8">
                                <select class="form-control input-sm" name="Period" style="width: 100%;" id="PeriodID">
                                    <option value="M">{{ getLanguage('Monthly') }}</option>
                                    <option value="Q">Quoterly</option>
                                </select>
                            </div>
                        </div> -->
