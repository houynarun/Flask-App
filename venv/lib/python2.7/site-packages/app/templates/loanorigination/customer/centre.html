				...
				<table class="table table-hover" >
					...
					<tbody>
						...
						<tr>
							...
							<td>{{toMoney(float(row.InterestRate),CurrencyObj(row.Currency))}}</td>
							<td>{{row.Term}}</td>
							...
							<td class="text-center">
								<a title="View Detail"  style="cursor:pointer; color:#1d89cf;"onclick="CustomClickView('LoanApplicationEx-{{row.ID}}','LoanApplicationEx/?ID={{row.ID}}')">
