...
{% block content %}
	...
	<div style="padding: 3% 0;">
		...
		<div class="col-sm-6" style="padding:20px 10px">
			<h3 class="header-3 text-center">Loan Write Off</h3>
			<form id="CustomDate" class="form-inline text-center" method="POST">
				<div class="form-group" style="margin-top:30px">
					<label for="FromDate" class="control-label">Date From:</label>
					<input class="input-sm form-control" type="text" placeholder="YYYY-MM-DD" name="FromDateWriteOff" id="FromDateWriteOff" value="{{ FromDateWriteOff | d('') }}">
					<label for="ToDate" class="control-label">To:</label>
					<input class="input-sm form-control" type="text" placeholder="YYYY-MM-DD" name="ToDateWriteOff" id="ToDateWriteOff" value="{{ ToDateWriteOff | d('') }}">
				</div>
			</form>
			<div class="text-center" style="margin:30px 0px;">
				<b>{{ getLanguage('Also available in') }} :</b>
				<a href="javascript:void(0);" id="WriteOffCSV" style="cursor:pointer; color:#1d89cf;" > <i class='fa fa-files-o'></i> CSV &nbsp;</a>|
				<a href="javascript:void(0);" id="WriteOffExcel" style="cursor:pointer; color:#1d89cf;"> <i class='fa fa-file-excel-o'></i> Excel &nbsp;</a>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			// Loan Writen Off
			$('#WriteOffExcel').on('click', function(event){
				FromDate = 'FromDate=' + $('input[name="FromDateWriteOff"]').val();
				ToDate   = 'ToDate=' + $('input[name="ToDateWriteOff"]').val();
				url = '/Morakot/CBCLoanWriteOff?Type=Excel' + '&' + FromDate + '&' + ToDate;
				getnerateDataEcel(url,'xlsx')
			});
			$('#WriteOffCSV').on('click', function(event){
				FromDate = 'FromDate=' + $('input[name="FromDateWriteOff"]').val();
				ToDate	= 'ToDate=' + $('input[name="ToDateWriteOff"]').val();
				url 	= '/Morakot/CBCLoanWriteOff?Type=CSV' + '&' + FromDate + '&' + ToDate;
				generateDataCSV(url, 'csv')
			});
		});
	</script>
	...
	<script type="text/javascript">
		$(function(){
			...
			init.push(function () {
				...
				$('#FromDateWriteOff').datepicker(options);
				$('#ToDateWriteOff').datepicker(options);
			});
		});
	</script>
