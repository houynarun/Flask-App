...
class ValidateSONextRunDate(SystemHealthCheck):

	def __init__(self):
		self.ItemName = "ValidateSONextRunDate"
		self.Title = "Standing Order Next Run Date"
		self.Status = "F"
		self.Level = "Critical"
		self.Code = "ACG-C004"
		self.Description = ""
	...
	def runValidate(self):
		...
		SOObj 		=	MKT_STANDING_ORDER.query.filter(MKT_STANDING_ORDER.NextRunDate < str(BankDate),
		...
		for row in BranchObj:
			...
			for col in CurrencyObj:
				...
				Message = []
				SOObjByBranch = SOObj.filter(MKT_STANDING_ORDER.Branch == row.ID , MKT_STANDING_ORDER.Currency == col.ID )

				if SOObjByBranch.all():
					LoanID = ["%s (%s)"%(item.ID,item.NextRunDate) for item in SOObjByBranch]
					Message = LoanID
