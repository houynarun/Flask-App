...
import app.tools.thread.mktgl			as mktgl
...
def getDefaultCurrency():
	...
	try:
		...
		CurrObj 	=	ThreadS.query(MKT_CURRENCY)
...
def getReportingRate(Currency,SystemDate="",Field="ReportingRate",ThreadS=ThreadS):
	try:
		if SystemDate:
			CurrencyObj = ThreadS.query(MKT_CURRENCY_DE).filter(MKT_CURRENCY_DE.Currency == Currency).\
			...
			if not CurrencyObj:
				...
				CurrencyObj = mktmoney.getCurrencyObj(Currency,ThreadS=ThreadS)
				# print "kCannot get exchange by date",SystemDate
			# else:
			# 	print CurrencyObj.SystemDate,CurrencyObj.ID
				
		else:
			# print "1Cannot get exchange by date",SystemDate
			CurrencyObj 	= mktmoney.getCurrencyObj(Currency,ThreadS=ThreadS)
			# print "11Cannot get exchange by date",SystemDate

		if CurrencyObj:
			# print "12Cannot get exchange by date",SystemDate
			ReportingRate 	= getattr(CurrencyObj,Field)
			# print "122Cannot get exchange by date",SystemDate
			if ReportingRate == 0:
				# print "1aCannot get exchange by date",SystemDate
				ThreadS.rollback()
...
def checkGLKey(Data, DrCrCategory, Currency, DrCr):
	...
	if mkttool.isContainWhitespace(Data):
		...
		if len(ListGLKey) != 12:
			...
			CategoryObj = ThreadS.query(MKT_CATEGORY).get(ListGLKey[0])
			...
			if ListGLKey[2]:
				AssetClassObj = ThreadS.query(MKT_ASSET_CLASS).get(ListGLKey[2])
...
def getConsolKeyBalance(ConsolKey="", Branch=""):
	try:
		...
		Consol = ThreadS.query(MKT_CONSOL_BALANCE).\
...
def getAccountRule(AccID=None,ThreadS=ThreadS):
	try:

		Acc = ThreadS.query(MKT_ACCOUNT).get(AccID)
		if Acc:
			AccProductID = str(Acc.AccProduct)
			AccProduct = ThreadS.query(MKT_ACC_PRODUCT).get(AccProductID)
			if AccProduct:
				AccRuleID = str(AccProduct.Rule) + str(Acc.Currency)
				AccRule = ThreadS.query(MKT_ACC_RULE_DE).get(AccRuleID)
				if AccRule:
					return AccRule
			else:
				return False
...
def getAccBalanceType(AccCategory,ThreadS=ThreadS):
	DrCr=''
	Category=ThreadS.query(MKT_CATEGORY).get(AccCategory)
	if Category:
...
def getAccAvailableBal(AccountID):
	try:
		...
		AvailableBal 	= 0
		Account 		= ThreadS.query(MKT_ACCOUNT).get(AccountID)
		if Account:
...
def insert_MKT_ACC_ENTRY(Status,Curr,Inputter,Createdon,Authorizer,
						...
						PrevBalance,UserReference,ThreadS=ThreadS):
...
def insert_MKT_JOURNAL(	Status, Curr, Inputter, Createdon, Authorizer,
						...
						Currency="", Branch="", GL_KEYS="", LCYAmount="", CustomerID="",Note="",UserReference="",Account="",ThreadS=ThreadS):
	...
	ConsolBal 		= 	ThreadS.query(MKT_CONSOL_BALANCE).\
...
def update_MKT_ACC(Acc, TranDate, Balance, AvailableBal, LastTran,ThreadS=ThreadS):

	try:
		Acc = ThreadS.query(MKT_ACCOUNT).get(Acc.ID)
		Acc.Balance 		= Balance
...
def calculateLCY(Currency=None, Amount=None, TransactionDate='',ThreadS=ThreadS):
	try:
		# print 'k1'
		ReportingRate = getReportingRate(Currency,ThreadS=ThreadS)
		# print 'k2'
...
def calculateBalance(AccCategory, Balance, Amount, DrCr,ThreadS=ThreadS):
	try:
		BalanceType = getAccBalanceType(AccCategory,ThreadS=ThreadS)
...
def calculateAvailableBal(AccIDID, Balance,ThreadS=ThreadS):
	try:

		AvailableBal = Balance
		Rule = getAccountRule(AccIDID,ThreadS=ThreadS)
		if Rule:
...
def getValidateConsolBalance(Amount, Category, Currency, Branch, DrCr, ConsolKey="", LCYAmount=0, ExchangeRate="",TranDate=""):
	try:
		...
		CategoryObj 		=	ThreadS.query(MKT_CATEGORY).get(Category)
		if CategoryObj:
			...
			if Category in [IncomeSummaryCat,CurrentYearCat,PriorYearCat]:
				BankDateObj 	=	mktdate.getBankDateObj(ThreadS=ThreadS)
	...
	try:
		...
		if AccountObj:
			if AccountObj.Branch != Branch:
				Branch = ThreadS.query(MKT_BRANCH).get(Branch)
...
def getValidationAccounting(Inputter="",Createdon="",Authorizer="",Authorizeon="",Branch="",DrAcc="",DrCat="",DrCur="",CrAcc="",CrCat="",CrCur="",Amount="",Module="",Tran="",TranDate="",Reference="",Note="",Mode=""):
	...
	try:
		Message=""
		CategoryList = [str(row.ID) for row in ThreadS.query(MKT_CATEGORY)]
		AccountObj   = ThreadS.query(MKT_ACCOUNT)
		if not Mode:
			if not DrAcc:
		...
		if not DrCat:
			...
			if mktteller.isCashCateogry(DrCat):
				if not DrAcc:
					CategoryDesc = DrCat
					CategoryObj = ThreadS.query(MKT_CATEGORY).get(DrCat)
					if CategoryObj:
		...
		if not CrCat:
			...
			if mktteller.isCashCateogry(CrCat):
				if not CrAcc:
					CategoryDesc = CrCat
					CategoryObj = ThreadS.query(MKT_CATEGORY).get(CrCat)
					if CategoryObj:
...
def setAccoutningCheckPoint(Mode,ID):
	try:
		...
		if not Mode: # Both Dr/Cr not Direct
			# Check Journal
			JournalObj = ThreadS.query(MKT_JOURNAL).filter(MKT_JOURNAL.Reference==ID)
			if JournalObj.all():
			...
			# Check Account Entry
			AccEntryObj = ThreadS.query(MKT_ACC_ENTRY).filter(MKT_ACC_ENTRY.Reference==ID)
			if AccEntryObj.all():
			...
			if ID[:2] == "MJ":
				...
				JournalSumObj = ThreadS.query(	func.sum(MKT_JOURNAL.Amount).label('Amount'),
				...
				CheckAccEntryObj = ThreadS.query(MKT_JOURNAL).filter(MKT_JOURNAL.Reference==ID).\
		...
		if float(Amount) > 0:
			...
			if Category in mktteller.getCashCategory():
				...
				if CashAccountObj[0]:
					...
					Balance 	= calculateBalance(Category,Balance, Amount, DrCr,ThreadS=ThreadS)
					# print "Balance%s"%Balance
					PrevBalance = AccountObj.Balance
					# print "PrevBalance%s"%PrevBalance
					AvailableBal = calculateAvailableBal(Account, Balance,ThreadS=ThreadS)
					...
					ObjUpdateAccountBalance = update_MKT_ACC(AccountObj, TransactionDate, Balance, AvailableBal, LastTran,ThreadS=ThreadS)
...
def getCustomerIDByAccount(Account,ThreadS=ThreadS):
	try:

		Customer 	= 	""

		Record 		=	ThreadS.query(
							MKT_ACCOUNT.ID,
...
def postAccounting(Status="", Curr="", Inputter="", Createdon="", Authorizer="", Authorizeon="", AEID="", Account="",Category="",
					...
					JNID="", Branch="", GL_KEYS="", Mode="", LastTran="No", LCYAmount=0, AllowEquivalent=True, UserReference="",IsMonthEnd=False,ThreadS=ThreadS):
	"""
	...
	try:
		...
		if LCYAmount == 0:	LCYAmount = calculateLCY(Currency, Amount,ThreadS=ThreadS)
		...
		processPostingAccounting(Status=Status, Curr=Curr, Inputter=Inputter, Createdon=Createdon, Authorizer=Authorizer, 
				...
				AllowEquivalent=AllowEquivalent, UserReference=UserReference,AccSetting=AccSetting,ThreadS=ThreadS)

		if TranDate <= LastMonthEnd:
			BranchObj 	=	ThreadS.query(MKT_BRANCH).with_for_update().get(Branch)
			...
			if BranchObj:
				...
				IsPreviousMonth		=	TranDate <= LastMonthEnd and TranDate > PrevLastMonthEnd and BankDate.year == TranDate.year
...
def processPostingAccounting(Status="", Curr="", Inputter="", Createdon="", Authorizer="", Authorizeon="", AEID="", Account="",
	...
	JNID="", Branch="", GL_KEYS="", Mode="", LastTran="No", LCYAmount=0, AllowEquivalent=True, UserReference="",AccSetting="",ThreadS=ThreadS):
	try:
		...
		if float(Amount) > 0:
			...
			if Mode == "Direct":
				...
				ObjInsertJN = insert_MKT_JOURNAL(Status, Curr, Inputter, Createdon, Authorizer,
											...
											TransactionDate, Reference, Currency, Branch, GL_KEYS, LCYAmount, CustomerID, Note, UserReference, Account=Account,ThreadS=ThreadS)
			else:

				Acc = ThreadS.query(MKT_ACCOUNT).with_for_update().get(Account)

				Balance = calculateBalance(Category,Acc.Balance, Amount, DrCr,ThreadS=ThreadS)
				...
				ObjInsertAE = insert_MKT_ACC_ENTRY(
											...
											Authorizeon, AEID, Account, Category, Currency,
											DrCr, Amount, Module,Transaction, TransactionDate,
											Reference, Note, Branch, PrevBalance,UserReference,ThreadS=ThreadS
											)
				CustomerID 	= getCustomerIDByAccount(Account,ThreadS=ThreadS)
				#Call function to insert into MKT_JOURNAL
				ObjInsertJN = insert_MKT_JOURNAL(Status, Curr, Inputter, Createdon, Authorizer,
												Authorizeon, JNID, Category, DrCr, Amount, Module, Transaction,
												TransactionDate, Reference, Currency, Branch, GL_KEYS, LCYAmount, CustomerID,Note, UserReference, Account=Account,ThreadS=ThreadS)
				# Call function to update_MKT_ACC
				ObjUpdateAccountBalance = update_MKT_ACC(Acc, TransactionDate, Balance, AvailableBal, LastTran,ThreadS=ThreadS)
			...
			ObjsetConsolBalance = mktgl.setConsolBalance(GL_KEYS, Branch, Currency, Amount, Category, DrCr, Module, TransactionDate, LCYAmount,ThreadS=ThreadS)
...
def isBSAccount(AccSetting,Category):
	...
	"""
	RevaluationCat 	= mktcurrencyrevaluate.getCategoryRevaluate(AccSetting,ThreadS=ThreadS) # Category of BS
...
def getReportingRateObj(ThreadS=ThreadS):
	
	Dict = {}
	CurrencyObj 	= ThreadS.query(MKT_CURRENCY)
	if CurrencyObj:
...
def getCategoryType(Category):
	try:
		...
		CategoryTypeObj 	=	ThreadS.query(MKT_CATEGORY).query.get(Category)
...
def isEquivalentPosting(Status,Curr,Inputter,Createdon,Authorizer,Authorizeon,Branch,Category,DebitCredit,ForeignCurrency,Amount,LCYAmount,Module,Transaction,TransactionDate,Reference,Note,CustomerID,AllowEquivalent):
	...
	if Revaluation == 'Y': # Check setting allow revaluation or not
		...
		if AllowEquivalent and ForeignCurrency != LocalCurrency: # Mean booking Equivalent and Position Account unless cross currency
			...
			if BSAccount:
				...
				setEquivalentPosting(Status,Curr,Inputter,Createdon,Authorizer,Authorizeon,Branch,AccSetting,DebitCredit,ForeignCurrency,Amount,LCYAmount,Module,Transaction,TransactionDate,Reference,Note,CustomerID)


def setEquivalentPosting(Status,Curr,Inputter,Createdon,Authorizer,Authorizeon,Branch,AccSetting,DebitCredit,ForeignCurrency,Amount,LCYAmount,Module,Transaction,TransactionDate,Reference,Note,CustomerID):
	...
	if DebitCredit == "Dr":
		...
		# Update Consolidate Balance
		ObjsetConsolBalance = mktgl.setConsolBalance(GL_KEYS, Branch, LocalCurrency, LCYAmount, Category, DrCr, Module, TransactionDate, LCYAmount,ThreadS=ThreadS)
		...
		# Update Consolidate Balance
		ObjsetConsolBalance = mktgl.setConsolBalance(GL_KEYS, Branch, ForeignCurrency, Amount, Category, DrCr, Module, TransactionDate, LCYAmount,ThreadS=ThreadS)
