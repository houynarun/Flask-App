...
def getNBCBankFieldSetting(NBCMFIField=None):
...
def getGLValueFromField(Branch = "ALL", Period = 2,NBCBankFieldSetting='',NBCBankFieldKey='', NBCExchangeRateDict={},**Kargs):

	GLValue = 0
	NBCBankFieldSetting = getNBCBankFieldSetting(NBCBankFieldSetting)
	
	if not NBCBankFieldSetting.has_key(NBCBankFieldKey):
		return  Decimal(GLValue)

	CurrencyObj     = MKT_CURRENCY.query.all()

	for Currency in CurrencyObj:
		GLVal    = mktgl.getGLValue(NBCBankFieldSetting[NBCBankFieldKey], Branch, Period, Currency.ID, **Kargs)
		GLValue    +=   Decimal(GLVal) * Decimal(NBCExchangeRateDict.get(Currency.ID,1)) / 1000000



	GLValue = Decimal(mktmoney.formatNumber(float(GLValue), 1, 8).replace(',',''))
	return  Decimal(GLValue)
