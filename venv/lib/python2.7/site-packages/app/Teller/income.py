...
@app.route("/Morakot/IncomePostingHotField", methods=['GET'])
def getIncomePostingHotField():
	return mktteller.getIncomeExpenseHotField('I')
...
class FRM_INCOME_POSTING(exform):

	TDrAccount 	= TextField(requiredlabel('Debit Account', "*"), [validators.Required("Till Account not found.")])
	DrCategory 	= TextField(requiredlabel("Debit Category", "*"), [validators.Required()])
	DrCurrency 	= QuerySelectField(requiredlabel('Currency', "*"),
	...
	TCrAccount 	= TextField(requiredlabel('Credit Account', "*"), [validators.Required("Suspend Account not found.")])
	CrCategory 	= TextField(requiredlabel("Credit Category", "*"), [validators.Required()])
	CrCurrency 	= TextField(requiredlabel("Credit Currency", "*"), [validators.Required()])
	...
	def validate_Amount(form, field):
		...
		Amo 		=	str(form.Amount.data).replace(',','')
	...
	@staticmethod
	def setVisible():
		control_list=['Transaction','TDrAccount','DrCategory','TCrAccount','CrCategory','CrCurrency']
		return control_list
	...
	def hotField():
		...
		varname 		= "Currency:$('#DrCurrency').val()"
		fun 			= ["DrCurrency", varname, fielddisplay, "/Morakot/IncomePostingHotField", "change"]
		hotfield.append(fun)
	...
	def beforeAuthorize():
		...
		try:
			...
			if not TellerObj:
				...
				flash(msg_error + "Teller Income posting %s not found." %ID)
				return False
				...
				CheckBalance = mktaccounting.checkMaxMinBalance(DrAccount, DrCurrency, Decimal(Amo), CheckBalanceDrCr)
				if CheckBalance:
				...
				# Call function to commit add account by currency 
				CheckAccounting = mktteller.setAuthorize('IP',MKT_TELLER,MKT_TELLER_INAU,ID)
				if not CheckAccounting[0]:
					db.session.rollback()
					db.session.close()
					flash(CheckAccounting[1])
	...
	def reverseRec():
		...
		try:
			...
			if not TellerObj:
				...
				return True, "%s Teller Income posting %s not found." %(msg_error, ID)
				...
				if not Check[0]:
					...
					Inputter	= Audit['Inputter']
					Createdon 	= Audit['Createdon']
					CheckAccounting = mktteller.setReverse('Teller',AuthLevel,'IP',MKT_TELLER,MKT_TELLER_INAU,ID,Inputter,Createdon)
