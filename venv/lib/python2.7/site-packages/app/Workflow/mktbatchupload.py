...
class CLS_BATCH_UPLOAD(object):
	...
	def bindFormAndTemplate(self,Form,Dict,Audit=""):
		"""
		def :
		...
		if Audit=="Y":
			...
			for Key in AuditKey:
				...
				for key in ListKey:
					if key !=Key and key !="Currency":
						del Dict[key]
		else:
			Obj 	=	eval(Form)
			for Key in Dict.keys():
				""" if has attr will return True else False"""
				if not hasattr(Obj,Key):
					if not Key in AuditKey:
						del Dict[Key]
		return Dict
	...
	def batchAuthorize(self,ID):
		try:
			...
			NAuthorize 			= 	self.getNAuthorize(URL='BatchUpload_I')
			if BatchObj:
				...
				if FormUrl:
					...
					if Status and isinstance(ListDict, (list,)):
						...
						for Dict in ListDict:
							...
							NewDict 	 = self.bindFormAndTemplate(Form,Dict,Audit=RemoveAudit)
							...
							MsgResponse ,status_code = mktapirequest.api_request.postForm(url=PostUrl,data=NewDict,Branch=Branch,Inputter=SelfInputter)
	...
	def flashMessage(self,Message,ID=""):
		...
		return ListMsg
	def autoAcceptOverride(self,Url,Data="",StaticMethod="",Operation="",Branch="",Inputter=""):
		if Operation=="New":
			api_request.postForm(Url,Data,Branch=Branch,Inputter=Inputter)
		else:
			pass
