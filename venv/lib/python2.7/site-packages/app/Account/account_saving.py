...
def getAccountProductSetup():
	VolunteeryProdCat 	=	mktsetting.getAppSetting('ACC_PRODUCT_VOLUNTARY')
	return MKT_ACC_PRODUCT.query.get(VolunteeryProdCat)
...
class FRM_ACCOUNT_SV(exform):
	...
	AccrInterest 	= 	DecimalField(getLanguage("Accrued Interest Payable"), [validators.Optional()], default=0, description="1-%s" %getLanguage("Other Info"))
	AccrCurMonth 	= 	DecimalField(getLanguage("Accr Int Current Month"), [validators.Optional()], default=0, description="1-%s" %getLanguage("Other Info"))
	...
	def beforeAuthorize():
		...
		if AccStatus == 'C':
			...
			AccObj.AccrInterest 	=	0
			AccObj.AccrCurMonth 	=	0
			AccObj.AccrCurCapital 	=	0
			AccObj.Balance 			=	0
			AccObj.AvailableBal 	=	0
